<template>
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Planet</h1>
        </div>
                ...
                <div class="card border-left-primary shadow h-100 py-2">
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <p class=" mb-1">Slider Kurzinfo Planet</p>
                            </div>
                            <div class="col-auto">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(heading , index) in planet.heading" :key="index">
                                <th scope="row">{{index+1}}</th>
                                <td>{{heading.title}}</td>
                                <td v-html="heading.description"></td>
                                <td>
        ...
        <b-modal ref="sliderWindow" size="lg" hide-footer title="Slider Kurzinfo Planet">
            <div class="modal-body">
                <form>
                    <div class="row">
                        <div class="col">
                            <input type="text" v-model="slider.name" class="form-control" placeholder="Name" required>
                            <div class="invalid-feedback" v-if="errors.name">{{errors.name[0]}}</div>
                        </div>
                        <div class="col">
                            <input type="text" v-model="slider.description" class="form-control" placeholder="Description" required>
                            <div class="invalid-feedback" v-if="errors.description">{{errors.description[0]}}</div>
                        </div>
                        <div class="col">
                            <input type="number" v-model="slider.order" class="form-control" placeholder="Choose order..." required>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(slider, index) in planet.slider" :key="index">
                            <th scope="row">{{index+1}}</th>
                            <td>{{slider.name}}</td>
        ...
        <!--start fakten window -->
        <b-modal ref="faktenWindow" size="lg" hide-footer title="Fakten Window...">
            <form>
                <div class="modal-body">
                    <div class="row">
                        <div class="col">
                            <input type="text" v-model="fakten.name" class="form-control" placeholder="Name" required>
                        </div>
                        <div class="col">
                            <input type="text" v-model="fakten.description" class="form-control" placeholder="Description" required>
                        </div>
                        <div class="col">
                            <input type="number" v-model="fakten.order" class="form-control" placeholder="Choose order" required>
                        </div>
                        <div class="col">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(fakten , index) in planet.fakten" :key="index">
                                <th scope="row">{{index+1}}</th>
                                <td>{{fakten.name}}</td>
        ...
        <!-- new window start -->
        <b-modal ref="newWindow" size="lg" hide-footer title="Heading...">
            <form v-on:submit.prevent="createHeading">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="">Title</label>
                        <input type="text" v-model="heading.title" class="form-control" aria-describedby="emailHelp" placeholder="Enter Title" required>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="file" id="file" ref="heading_file" v-on:change="attachImage" class="form-control-file" required>
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Description</label>
                        <vue-editor v-model="heading.description"></vue-editor>
                    </div>
                <br>
                </div>
                    <div class="modal-footer">
        ...
        <b-modal ref="editNewWindow" size="lg" hide-footer title="Edit Uberschrift...">
            <form v-on:submit.prevent="updateHeading">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="">Title</label>
                        <input type="text" v-model="editHeadingData.title" class="form-control" aria-describedby="emailHelp" placeholder="Enter Title" required>
                    </div>
                    <div class="form-group">
                        <label for="marsFile">Video | Image</label>
                        <div>
                        ...
                        <input id="image" type="file" ref="heading_file" v-on:change="editAttachFile" class="form-control-file" required>
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Description</label>
                        <vue-editor  v-model="editHeadingData.description"></vue-editor>
                    </div>
                <br>
                </div>
                    <div class="modal-footer">
        ...
        <!-- Seo Window -->
        <b-modal ref="seoWindow" size="lg" hide-footer title="Mars Seo Window....">
            <form>    
                <div class="modal-body">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" v-model="seo.meta_tags" class="form-control" id="exampleInputEmail1"
                            aria-describedby="emailHelp" placeholder="Enter meta tags" required>
                        <div class="invalid-feedback" v-if="errors.meta_tags">{{errors.meta_tags[0]}}</div>
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Short Description</label>
                        <textarea v-model="seo.description" class="form-control" rows="7" required></textarea>
                        <div class="invalid-feedback" v-if="errors.description">{{errors.description[0]}}</div>
                    </div>
                <br>
                </div>
                <div class="modal-footer">
                    <button type="submit" v-on:click.prevent="updateSeo" class="btn btn-primary">Update</button>
                    <button type="button" v-on:click="hideSeoWindow" class="btn btn-secondary">Close</button>
                    <!-- <div v-if="this.planet.seo">
                        <button type="submit" v-if="this.planet.seo.created_at" v-on:click.prevent="updateSeo" class="btn btn-primary">Update</button>
                    </div> -->
        ...
        methods: {
            ...
            updateHeading: async function () {
                ...
                try {
                    ...
                    this.planet.heading.map(heading => {
            ...
            deleteHeading: async function (heading){
                ...
                try {
                    ...
                    this.planet.heading = this.planet.heading.filter(obj => {
            ...
            deleteSlider: async function (slider){
                ...
                try {
                    ...
                    this.planet.slider = this.planet.slider.filter(obj => {
            ...
            deleteFakten: async function (fakten){
                ...
                try {
                    ...
                    this.planet.fakten= this.planet.fakten.filter(obj => {
            ...
            deleteSeo: async function (seo){
                if (
                    !window.confirm(`Are you sure you want to delete ${seo.title}`)
                ) {
                    return;
                }
                try {
                    const response = await planetService.deleteSeo(seo.id);
                    this.planet.seo = this.planet.seo.filter(obj => {
